...
fitVSEM <- function(fname,iter=20000,params=refPars){
...
vsemDiagnostics <- function(out,refPars){
  ...
  for(i in 1:3){
    ...
    plotTimeSeriesResults(x = out, model = runModel, observed = myObs, error = errorFunction,pool=i,start=nmc/2)
...
vsemDiagnosticsEsys<- function(out,refPars){
  ## thin
  nmc = nrow(out$chain[[1]])
  ## out$chain <- window(out$chain,start=nmc/2,thin=(nmc/2)/5000)
  ## could replace above with getSample?
  ...
  for(i in 1:3){
    myObs = obs[,i]
    if(exists("obsSel") & i %in% isLow) myObs[-obsSel] <- NA
    plotTimeSeriesResults(x = out, model = runModelEsys, observed = myObs,pool=i,start=nmc/2)
    lines(referenceData[,i],col=3,lwd=1)
  }
...
knitr::kable(skill[[9]],digits=3)
obs <- obs.orig
```

### Perfect model balanced data that has a multiplicative bias 
Cs obs have a multiplicative error of times 2
...
out11 <- fitVSEM("run11.RData")
```

```{r echo=FALSE, background='white', fig.height=7, fig.cap="Perfect model, biased data"}
vsemDiagnostics(out11,refPars)
obs <- obs.orig
skill[[10]] <-vsemScore(out11) 
knitr::kable(skill[[10]],digits=3)
```

### Perfect model unbalanced data that has a multiplicative bias 
Cs obs have a multiplicative error of times 2
...
```{r echo=FALSE, background='white', fig.height=7, fig.cap="Biased, unbalanced data"}
vsemDiagnostics(out12,refPars)
skill[[11]] <-vsemScore(out12) 
knitr::kable(skill[[11]],digits=3)
obs <- obs.orig
```

### Perfect model low-volume data that has a multiplicative bias 
Cs obs have a multiplicative error of times 2
...
```{r echo=FALSE, background='white', fig.height=7, fig.cap="Biased low-volume data"}
vsemDiagnostics(out13,refPars)
skill[[12]] <-vsemScore(out13) 
knitr::kable(skill[[12]],digits=3)
obs <- obs.orig
```
...
out8 <- fitVSEM("run8.RData", params=addPars)
```
...
obs <- obs.orig
skill[[13]] <-vsemScore(out8) 
knitr::kable(skill[[13]],digits=3)
obs <- obs.orig

```

### Perfect model unbalanced data that has a multiplicative bias with systmatic bias parameter
Cs obs have an multiplicative error of times 2
Gaussian 1000 obs for NEE and Cs and only 6 for Cv
...
addPars <- refPars
addPars[nvar+1,] <- c(2.0, 0.0, 4.0)
row.names(addPars)<-c(row.names(refPars),"EsysCs")
newPars <- addPars$best
parSel = c(defParms, nvar,nvar+1)
obsSel <- c(1,202,390,550,750,920)
isLow = 2
...
out14 <- fitVSEM("run14.RData", params=addPars)
```

```{r echo=FALSE, background='white', fig.height=7, fig.cap="Biased, unbalanced data; multiplicative bias likelihood"}
vsemDiagnostics(out8,addPars)
obs <- obs.orig
skill[[14]] <-vsemScore(out14) 
knitr::kable(skill[[14]],digits=3)
obs <- obs.orig
...
### Model with error unbalanced perfect data with additive and multiplicative model error parameters
```{r include=FALSE}
obs <- obs.orig
...
newPars["Av"]  <- 1.0
newPars["Cr"] <- 0.0
## parSel = c(defParms, nvar,nvar+1,nvar+2)
...
likelihood <- function(x, sum = TRUE){
  ...
  llValues1  <- dnorm(diff, sd = (abs(c(obs[,1])) + 0.0000001) * x[nvar], log = T) 
  
  diff       <- c(predicted[obsSel,2] - obs[obsSel,2])
  llValues2  <- dnorm(diff, sd = (abs(c(obs[obsSel,2])) + 0.0000001) * x[nvar], log = T) 

  diff       <- c(((predicted[1,3] + (predicted[,3] - predicted[1,3])*x[nvar+2]) + x[nvar+4]) - obs[,3]) 
  llValues3  <- dnorm(diff, sd = (abs(c(obs[,3])) + 0.0000001) * x[nvar], log = T)
...
prior         <- createUniformPrior(lower = addPars$lower[parSel], upper = addPars$upper[parSel])
out15 <- fitVSEM("run15.RData",iter=40000, params=addPars)
```

```{r echo=FALSE, background='white', fig.height=7, fig.cap="Model error, unbalanced data; additive + multiplicative bias likelihood"}
vsemDiagnosticsEsys(out15,addPars)
obs <- obs.orig
skill[[15]] <-vsemScore(out15) 
knitr::kable(skill[[15]],digits=3)
obs <- obs.orig
