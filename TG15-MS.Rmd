...
author: "David Cameron, Florian Hartig, Francesco Minnuno, Bjoern Reinking, Marcel Van Oijen and Mike Dietze"
date: '`r Sys.Date()`'
output:
...
In addition to considering model structural error, we also wish to investigate the influence of observations with biases since all observational data will to a greater or lesser extent contain biases. Here we simulate data biases by multiplying the soil data by two to represent a considerable multiplicative bias in the observations of soil carbon. 
...
## Identifying the issue including unbalanced data in Bayesian calibration.

 * Calibration with a perfect model and unbalanced data has demonstrated that there is no intrinsic issue with unbalanced data. 
 * Model error is introduced first with balanced data. We do not normally know the true parameter settings so in terms of model predictions would be regarded as a successful calibration.
 * The goal ‘from the perspective of the calibration’ is to diminish model-data difference. 
 ...
 * There is quite a lot of data present in calibration so the result, in terms of the parameters, is confidently wrong. 
 * It is when the model has an error and data is unbalanced that we start to see an issue in the model predictions versus data.
 * Now after calibration the model predictions are also confidently wrong. 
 ...
 * From its 'own perspective' the calibration is 'doing the right thing' as there is lots of evidence in the likelihood that the model is off from soil and NEE data but only a little evidence that model is far away from vegetation data.
 * These idealised calibrations have demonstrated that the issue is with the model error causing a model-data difference where the only means of reducing the difference is for the parameters to depart from their true value.
 * The root issue is the presence of the model error not the imbalance in calibration data. 
